...
Untuk views, saya lakukan hal yang sama dengan tutorial minggu lalu untuk menambahkan XML dan JSON. Untuk itu, saya tinggal menghubungkan fungsi yang telah saya buat kepada URL. Lalu tinggal saya lakukan `python manage.py runserver 8000` untuk cek apakah sudah benar atau belum. Apabila sudah benar, lalu saya mengecek `localhost:3000/mywatchlist/html`, `localhost:3000/mywatchlist/json`, dan `localhost:3000/mywatchlist/xml`. Apabila sudah benar, saya coba melakukan testing dengan menggunakan kode saya pada tugas 2 (bonus). Lalu saya `python manage.py test` untuk mengecek apabila testing sudah benar atau belum. Saya juga mengerjakan bonus yang diberikan yaitu mengecek apabila jumlah watched lebih banyak daripada yang belum di tonton. Saya menggunakan filter untuk melakukan hal ini. Lalu pada filter tersebut saya menghitung berapa panjang data yang didapat lalu saya bandingkan dengan data yang belum tonton. Ketika sudah berhaisl Saya kemudian melakukan pushing kepada git menggunakan `git status`, `git add .`, `git commit -m ".."`, dan `git push origin main`.

Setelah itu saya coba cek heroku saya untuk memastikan bahwa deployment berhasil. Lalu saya mengerjakan read me ini :D.
